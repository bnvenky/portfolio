...
export function Project() {
  ...
  return (
    <Section id="project" bg>
      ...
      <div style={{ background:"rgba(0,212,255,0.02)",border:"1px solid rgba(0,212,255,0.22)",borderRadius:24,padding:isMobile?"22px":"44px",position:"relative",overflow:"hidden",transition:"border-color .3s" }}
        ...
        <div style={{ position:"absolute",top:-60,right:-60,width:240,height:240,borderRadius:"50%",background:"radial-gradient(circle,rgba(0,212,255,0.06),transparent 70%)",pointerEvents:"none" }}/>
        {/* Header */}
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:12,marginBottom:20 }}>
          <div>
            <div style={{ display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",marginBottom:6 }}>
              <h3 style={{ fontFamily:"Syne, sans-serif",fontWeight:800,fontSize:isMobile?22:28,color:"#fff" }}>{p.name}</h3>
        ...
        <div style={{ display:"flex",flexWrap:"wrap",gap:8,marginBottom:28 }}>
          {p.stack.map(s => <span key={s} style={{ padding:"5px 13px",borderRadius:50,background:"rgba(0,212,255,0.08)",border:"1px solid rgba(0,212,255,0.22)",color:"#00d4ff",fontSize:12,fontWeight:600 }}>{s}</span>)}
        </div>
        {/* Bullets */}
